...
class fun(commands.Cog):
    ...
    async def hack(self, ctx, user:discord.User):
        ...
        embed1 = discord.Embed(title="Fetching ip...", color=discord.Colour.cyan())
        msg = await ctx.reply(embed=embed1)
        await asyncio.sleep(2)
        ipemb=discord.Embed(title=f"ip: {ip}", color=discord.Colour.cyan())
        await msg.edit(embed=ipemb)
        await asyncio.sleep(1.5)
        embed2=discord.Embed(title="Looking for mom...", color=discord.Colour.cyan())
        await msg.edit(embed=embed2)
        await asyncio.sleep(2)
        momnameemb = discord.Embed(title=f"Mom's name: {momname}", color=discord.Colour.cyan())
        await msg.edit(embed=momnameemb)
        await asyncio.sleep(1.5)
        embed3=discord.Embed(title=f"`{user}.fetch_dms()`...", color=discord.Colour.cyan())
        await msg.edit(embed=embed3)
        await asyncio.sleep(2)
        lastdmemb = discord.Embed(title=f"Last DM: \"{lastdm}\"", color=discord.Colour.cyan())
        await msg.edit(embed=lastdmemb)
        await asyncio.sleep(1.5)
        embed4=discord.Embed(title=f"`{user}.favorite_food`...", color=discord.Colour.cyan())
        await msg.edit(embed=embed4)
        await asyncio.sleep(2)
        favfoodemb=discord.Embed(title=f"Favorite food: {favoritefood}", color=discord.Colour.cyan())
        await msg.edit(embed=favfoodemb)
        await asyncio.sleep(1.5)
        embed5=discord.Embed(title=f"Getting crush...", color=discord.Colour.cyan())
        await msg.edit(embed=embed5)
        await asyncio.sleep(2)
        crushemb=discord.Embed(title=f"Crush: {crush}", color=discord.Colour.cyan())
        await msg.edit(embed=crushemb)
        await asyncio.sleep(2)
        embed6=discord.Embed(title="The totally real hack just finished.", color=discord.Colour.cyan())
